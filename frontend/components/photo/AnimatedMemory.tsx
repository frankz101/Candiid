...
const AnimatedMemory = ({
    ...
    const fetchHangout = async () => {
        return axios
            .get(
                `${process.env.EXPO_PUBLIC_API_URL}/user/${user?.id}/post/${postId}`
            )
            .then((res) => res.data);
    };
        ...
        return () => {
            if (!isPhotoVisible) {
                queryClient
                    ...
                    .then(() => {
                        console.log(
                            "Memories data refreshed after deleting a photo."
                        );
                    });
            }
        };
                    ...
                    {postId && photoData ? (
                        <View
                            style={[
                                ...
                                viewStyles[frame].imageContainer,
                            ]}
                        >
                            <Animated.View>
                                <Image
                                    source={{
                                        uri: photoData.photoUrls[0].fileUrl,
                                    }}
                                    style={[
                                        styles.baseImageStyle,
                                        viewStyles[frame].image,
                                    ]}
                                />
                            </Animated.View>
                        </View>
